...
test.describe('API tests', { tag: '@api' }, () => {
  test('getBlogsBySlug API validation', async ({}) => {
    ...
    const blankSlugValue = await apiContext.get(
      `${process.env.NEXT_PUBLIC_BASE_URL}/api/blogs/slug/`
    );
    expect(blankSlugValue.status()).toBe(StatusCodes.NOT_FOUND);

    const slugAsNumber = await apiContext.get(
      `${process.env.NEXT_PUBLIC_BASE_URL}/api/blogs/slug/${1}`
    );
    expect(slugAsNumber.status()).toBe(StatusCodes.BAD_REQUEST);

    const invalidSlug = await apiContext.get(
      `${process.env.NEXT_PUBLIC_BASE_URL}/api/blogs/slug/invalid slug`
    );
